...
import com.gmlive.svgaplayer.util.isMainThread
...
internal class ViewTargetRequestManager : View.OnAttachStateChangeListener {
    ...
    @Volatile
    var metadata: SVGAResult.Metadata? = null
    ...
    private val videoEntities = SimpleArrayMap<Any, SVGAVideoEntity>()

    /** Associate [bitmap] with [tag] and cache it on this view. Return the previous bitmap associated with [tag]. */
    @MainThread
    fun put(tag: Any, entity: SVGAVideoEntity?): SVGAVideoEntity? {
        return if (entity != null) {
            videoEntities.put(tag, entity)
        } else {
            videoEntities.remove(tag)
        }
    }
